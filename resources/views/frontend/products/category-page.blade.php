        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-12 gap-10">
                <div class="col-span-3">
                    <div class="bg-gray-50 shadow p-5">
                        <h3 class="text-m uppercase pb-3 font-bold">Categories</h3>
                        <ul>
                            @foreach ($cats as $item)
                                <li class="text-m pb-2 font-semibold text-gray-600">
                                    <a
                                        href="{{ route('category.page', ['main_cat' => $main_cat, 'sub_cat' => $item->slug]) }}">
                                ...
                                <a href="{{ route('product.page', ['slug' => $item->slug]) }}">
                                    ...
                                    <div class="relative overflow-hidden">
                                        <div class="text-sm px-5 pt-3 font-semibold text-center text-gray-600">Size:
                                            ...
                                            {{ $item->color->name }}</div>
                                        <div class="text-sm px-5 pb-3 font-semibold text-center ">
                                            @if ($item->stock_quantity > 0)
                                                <span class="text-lime-600">{{ $item->stock_quantity }} in stock</span>
                                            @else
